...
class DamOptimizer:
    ...
    def _normalize_volume(self, q_blocks: np.ndarray) -> np.ndarray:
        ...
        최적화 후 결과에 적용하여 총량 보존 강제.
        블록별 비율을 유지하면서 합계를 원본과 맞춤.
        """
        ...
        return np.clip(q_blocks * scale, self.q_min, self.q_max)
    ...
    def _objective(self, q_blocks: np.ndarray,
                   w_penalty: float, w_volume: float) -> float:
        ...
        total     = w_penalty * ds + w_volume * pv + pr
        # 수렴 이력 저장 (DS만 기록)
    ...
    def optimize(self, scenario: str = "Moderate",
        ...
        def obj(q):
            return self._objective(
                np.clip(q, self.q_min, self.q_max), w_p, w_v
            )

        # ── 시나리오별 초기값 차별화 ──────────────────────────── #
        # Conservative: 초기에 많이 방류 (사전 방류 유도)
        # Moderate:     원본 그대로
        # Aggressive:   초기에 적게 방류 (저수지 수위 유지)
        x0 = self.q_init_blocks.copy()
        if scenario == "Conservative":
            # 앞쪽 블록에 가중치 → 사전 방류
            ramp = np.linspace(1.3, 0.7, self.n_blocks)
            x0   = np.clip(x0 * ramp, self.q_min, self.q_max)
        elif scenario == "Aggressive":
            # 뒤쪽 블록에 가중치 → 후반 방류
            ramp = np.linspace(0.7, 1.3, self.n_blocks)
            x0   = np.clip(x0 * ramp, self.q_min, self.q_max)
