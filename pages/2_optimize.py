...
total_h = int(n_blocks) * block_min / 60
...
| 항목 | 값 |
|------|-----|
| 방류량 조절 횟수 | **{bi['n_blocks']}** 블록 |
| 블록 길이 | **{bi['block_hours']:.1f}** h |
| 최적화 기간 | **{bi['total_hours']:.0f}** h = **{bi['total_days']:.1f}** 일 |
| 커버리지 | **{bi['coverage']*100:.0f}%** |
...
q_scaled = q_sj * (block_scale / 100.0)
n_t      = len(q_sj)
