...
with st.expander("📦 데이터 준비"):
    st.markdown("""
**데모 모드 (NC 파일 없이)**: `data/tesr_demo.npz` 등 내장 데이터 자동 사용

**NC 파일 사용**: 사이드바에서 `.nc` 파일 업로드  
(K-Water HEC-RAS 출력 형식: `K-RIVER/Output/Station/WL`, `Q`)

**로컬 NC 경로**: `data/tesr.nc`에 복사하면 자동 인식
""")
