...
with st.sidebar:
    ...
    scenario_mode = st.radio("모드", ["시나리오 선택", "직접 입력"],
                             horizontal=True)
    if scenario_mode == "시나리오 선택":
        ...
        scenario_name = None
        custom_scale  = st.slider(
            "유량 배율 (1.0 = 기준)", 0.5, 2.0, 1.0, 0.05,
            format="×%.2f"
        )
...
st.caption(f"방류 최적화 시뮬레이터 — 기준(최초) vs Conservative / Moderate / Aggressive{scale_tag}")
...
try:
    nc = load_with_scenario(event, scenario_name, custom_scale)
