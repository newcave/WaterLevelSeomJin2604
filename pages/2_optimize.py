...
with st.sidebar:
    ...
    block_min_opt = st.selectbox(
        "구간 단위 [분]",
        ["자동 (n_time 균등 분할)", "330분 (5.5h)", "180분 (3h)",
         "360분 (6h)", "720분 (12h)"],
        index=0,
        help="자동: 전체 기간을 n_blocks 등분 → 5일 / 20블록 = 6h/블록"
    )
    # 자동이면 None → optimizer가 n_time 기반 계산
    _bm_map = {"자동 (n_time 균등 분할)": None,
               "330분 (5.5h)": 330, "180분 (3h)": 180,
               "360분 (6h)": 360, "720분 (12h)": 720}
    block_min = _bm_map[block_min_opt]
...
cfg_override = {
    ...
    "block_minutes": block_min,   # None = 자동, 숫자 = 고정
